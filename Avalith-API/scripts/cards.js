...
const printPagination = (info) => {
  let body = `<div class="btn-container"><button onclick="getData('${
    info.prev
  }'); topFunction(); onClickedPrev()" class =${
    info.next == "https://rickandmortyapi.com/api/character/?page=2" && "noprev"
  }>Previous</button></div>`;
  body += `<div class="btn-container"><button onclick="getData('${
    info.next
  }');  topFunction();  onClickedNext()"  class =${
    info.prev == "https://rickandmortyapi.com/api/character/?page=41" &&
    "noprev"
  }> Next</button></div>`;
